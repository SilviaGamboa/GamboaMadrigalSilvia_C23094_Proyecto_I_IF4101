...
document.addEventListener("DOMContentLoaded", () => {
  ...
  formulario.addEventListener("submit", async (event) => {
    ...
    const esValido =
      ...
      validarTelefono(numtelefono) &&
      validarCorreo(correo) &&
      validarContrasenna(contrasenna) &&
      confirmarContrasenna(contrasenna, confirmContrasenna);
    ...
    if (esValido) {
      ...
      if (
        gestionarUsuarios.validarDatosRepetidos(
          usuario._cedula,
          usuario._numTelefono,
          usuario._correo
        )
      ) {
...
const obtenerDatosFormulario = () => {
  ...
  const confirmContrasenna = document
    .getElementById("confirmContrasenna")
    .value.trim();
  return {
    cedula,
    nombre,
...
const limpiarCamposTexto = () => {
  const campos = document.querySelectorAll(
    "#formulario input[type='text'], #formulario input[type='number'], #formulario input[type='tel'], #formulario input[type='email'], #formulario input[type='password']"
  );
