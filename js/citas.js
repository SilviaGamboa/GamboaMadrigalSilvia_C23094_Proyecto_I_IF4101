document.addEventListener("DOMContentLoaded", function () {
      ...
      if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        ...
        for (let i = 0; i < medicos.length; i++) {
          const especialidadMedico =
            medicos[i].getElementsByTagName("especialidad")[0].textContent;

          if (especialidadMedico.includes(opcionSeleccionada)) {
            const nombreMedico =
              medicos[i].getElementsByTagName("nombre")[0].textContent;
  ...
  addEventSubmit.addEventListener("click", () => {
    ...
    eventsArr.forEach((event) => {
      if (
        ...
        event.events.forEach((event) => {
          if (event.title === eventTitle || event.horaFormato === hora) {
            if (event.usuario._cedula === usuariocomp._cedula) {
              eventExist = true;
            } else {
              if (event.medico === medico && event.horaFormato === hora) {
                medicoOcupado = true;
                console.log(event.medico, medico, event.horaFormato, hora);
              }
            }
          }
        });
    ...
    if (medicoOcupado) {
      alert("El medico no esta disponible a esa hora ");
      return;
    }
    ...
    function compruebaHorario() {
      for (let i = 0; i < medicos.length; i++) {
        if (
          medico === medicos[i].getElementsByTagName("nombre")[0].textContent
        ) {
          const horariodMedico = medicos[i]
            .getElementsByTagName("horariosConsulta")[0]
            .textContent.trim();
        ...
        if (
          ...
          event.events.forEach((item, index) => {
            if (
              ...
              item.usuario._cedula === usuariocomp._cedula
            ) {
              ...
              const confirmaBtn = document.createElement("div");
              confirmaBtn.id = "confirmaCita-btn";
              confirmaBtn.innerHTML = "confirmar Cita";

              const modificarBtn = document.createElement("div");
              modificarBtn.id = "modificarCita-btn";
              modificarBtn.innerHTML = "modificar";

              const aceptarBtn = document.createElement("div");
              aceptarBtn.id = "aceptarModificacion-btn";
              ...
              content.innerHTML = `<h2>Informacion completa de la cita</h2> 
                                          ...
                                          <h3> Cedula Paciente</h3><p>${item.usuario._cedula}</p>
                                          <h3> Confima Usuario</h3><p>${item.confirmaU}</p>
                                          <h3> Confirma Medico: </h3><p>${item.confirmaMed}</p> `;
              ...
              container.appendChild(confirmaBtn);
  ...
  //obtenereventos del localstorage
  function getEvents() {
  ...
  function medicosDisponibles(horarioTexto, diaSelec, horaSelec) {
    ...
    if (
      (diaSelec === "Lunes" && horarioTrabajo.lunes) ||
